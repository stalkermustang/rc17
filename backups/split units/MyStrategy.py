    ...
    def baseBind(self, me, world, game, move):
        if world.tick_index == 0:
            ...
            for i in ['TANK', 'IFV', 'ARRV', 'FIGHTER', 'HELICOPTER']:
                ...
                group_counter+=1
                self.delayedMoves.put('self.g_x_c, self.g_y_c = self.getCenterOfSelected(); move.action = model.ActionType.ActionType.CLEAR_AND_SELECT; move.right = self.g_x_c; move.left = self.g_x_c-27; move.top = self.g_y_c-27; move.bottom = self.g_y_c')
                self.delayedMoves.put('move.action = model.ActionType.ActionType.ASSIGN; move.group = ' + str(group_counter))
                self.delayedMoves.put('move.action = model.ActionType.ActionType.SCALE; x_c, y_c = self.getCenterOfSelected(); move.x = x_c; move.y = y_c; move.factor = 0.5')

                group_counter+=1
                self.delayedMoves.put('move.action = model.ActionType.ActionType.CLEAR_AND_SELECT; move.right = self.g_x_c+27; move.left = self.g_x_c; move.top = self.g_y_c-27; move.bottom = self.g_y_c')
                self.delayedMoves.put('move.action = model.ActionType.ActionType.ASSIGN; move.group = ' + str(group_counter))
                self.delayedMoves.put('move.action = model.ActionType.ActionType.SCALE; x_c, y_c = self.getCenterOfSelected(); move.x = x_c; move.y = y_c; move.factor = 0.5')

                group_counter+=1
                self.delayedMoves.put('move.action = model.ActionType.ActionType.CLEAR_AND_SELECT; move.right = self.g_x_c; move.left = self.g_x_c-27; move.top = self.g_y_c; move.bottom = self.g_y_c+27')
                self.delayedMoves.put('move.action = model.ActionType.ActionType.ASSIGN; move.group = ' + str(group_counter))
                self.delayedMoves.put('move.action = model.ActionType.ActionType.SCALE; x_c, y_c = self.getCenterOfSelected(); move.x = x_c; move.y = y_c; move.factor = 0.5')

                group_counter+=1
                self.delayedMoves.put('move.action = model.ActionType.ActionType.CLEAR_AND_SELECT; move.right = self.g_x_c+27; move.left = self.g_x_c; move.top = self.g_y_c; move.bottom = self.g_y_c+27')
                self.delayedMoves.put('move.action = model.ActionType.ActionType.ASSIGN; move.group = ' + str(group_counter))
                self.delayedMoves.put('move.action = model.ActionType.ActionType.SCALE; x_c, y_c = self.getCenterOfSelected(); move.x = x_c; move.y = y_c; move.factor = 0.5')
                
                group_counter+=1

            #пробиндили все группы
